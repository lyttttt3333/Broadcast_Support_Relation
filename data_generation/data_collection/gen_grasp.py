...
class clutter_scene():
    def __init__(self, input_args) -> None:
        ...
        self.grid = 2
        self.gripper_path = "omniverse://localhost/Users/sim/frank_gripper.usd"
    ...
    def build_env(self):
        ...
        self.logger=LogInteraction(self.indice,self.save_path,structure_path,None,device=self.device)
        self.target_pts, self.target_pts_normals, self.target_rigid = get_target_pts(root_dir=self.dir_path, index=self.indice, save_path = self.save_path) 
        self.total_division = 9
    ...
    def simulation(self):
        ...
        for rigid_idx in range(len(self.target_rigid)):
            ...
            rigid_normals = self.target_pts_normals[rigid_idx]
            ...
            for batch_idx in range(batch_num):
                ...
                batch_normals = rigid_normals[self.env_num * batch_idx: self.env_num * (batch_idx + 1)]

                controller.create_buffer()

                for pose_sample_idx in range(self.total_division):
                    

                    controller.set_gripper_pose(grid=self.grid,env_line=self.env_line,
                                                target_point=batch_pts,
                                                target_normal=batch_normals,
                                                idx=pose_sample_idx,
                                                total_division=self.total_division
                                                )
...
if __name__ == "__main__":
    ...
    argparser.add_argument("--gravity", type=float, default=-0.98)
