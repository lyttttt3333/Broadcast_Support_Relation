...
def find_batch_outer_env_obj(root_path, train:bool):
    idx = find_all_ids(root_path=root_path,train=train)
    outer_env_list=[]
    for i in idx:
        outer_env_list+=find_batch_inner_env_obj(root_path, i, train)
    return outer_env_list

def find_all_ids(root_path, train:bool):
    id_list=[]
    if train:
    ...
    for item in os.listdir(path):
        id_list.append(int(item[4:]))
    return id_list

def find_batch_inner_env_obj(root_path, id:int,train:bool):
    if train:
        path=os.path.join(root_path, "train", f"env_{id}")
    else:
        path=os.path.join(root_path, "test", f"env_{id}")
    inner_env_list=[]
    all_need_path=[]
    for item in os.listdir(path):
        if item.split(".")[-1] == "npy":
...
def find_relation(state_info_list,batched_mask):
    # output a mat [B,part,part] 1 for connected 0 for not connected ( a force b )
    B=len(state_info_list)
    relation_mat_list=[]
    for i in range(B):
        state_info=state_info_list[i]
        relation_mat=torch.zeros(state_info.shape[0]).to("cuda:0")
        mask=batched_mask[i]
...
def merget_pairs(motion,relation,partpoint_list,state_list,action_list,pair,output_path,train:bool, idx:int):
    import os
    if train:
        succ_path = os.path.join(output_path,"pair_train","succ")
        fail_path = os.path.join(output_path,"pair_train","fail")
    else:
        succ_path = os.path.join(output_path,"pair_test","succ")
        fail_path = os.path.join(output_path,"pair_test","fail")
    for i in range(len(partpoint_list)):
        ...
        for indice in indices_to_save:
            ...
            if kinetic.item() == 1:
                save_data[-1,:]=np.array([0,0,0])
                save_data[-2,:]=action_succ.cpu().numpy()
                save_path = os.path.join(fail_path,f"{10000+idx}.npy")
                np.save(save_path,save_data)
            else:
                save_data[-1,:]=np.array([1,0,0])
                save_data[-2,:]=action_succ.cpu().numpy()
                save_path = os.path.join(succ_path,f"{10000+idx}.npy")
                np.save(save_path,save_data)
        idx += 1
    return idx
